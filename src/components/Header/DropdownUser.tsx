...
import { clearAccountData } from "@/redux/slices/authSlice";
...
const DropdownUser = () => {
  ...
  const handleOkay = async () => {
    setLoading(true);
    let response = await dispatch(getLogout());
    setLoading(false);
    if (response?.success) {
      dispatch(clearAccountData());
      dispatch(storeError("You are logged outed"));
    }
      ...
      <ClickOutside onClick={() => setDropdownOpen(false)} className="relative">
        ...
        >
          <span className="hidden text-right lg:block">
            ...
            <span className="block text-xs">{`${account?.user?.roleType}`}</span>
          </span>
